#!/usr/bin/env python3
"""Render the three naval units (Rowing Boat, Sailing Boat, Frigate) from the
user's FBX models into 2D sprites, one per tribe. Blank (~0.8 grey) vertex
regions are recoloured per tribe; all other painted vertex colours are kept.

All three share ONE world scale + a shared union crop, so their canvases are
identical (uniform on-map placement) and their relative sizes are correct.

Prereq: assimp export <name>.fbx /tmp/model/<name>.gltf  (gltf + bin)
Models keyed by naval tier used in the game engine.
"""
...
# gltf name -> naval tier (asset folder / prefix used by the UI)
MODELS = {"rowing_boat": "rowing", "sailing_boat": "sailing", "frigate": "battleship"}
...
def load(name):
    ...
    # Normalise orientation so the hull LENGTH is along X for every model.
    flat = R.reshape(-1, 3)
    xext = flat[:, 0].max() - flat[:, 0].min()
    yext = flat[:, 1].max() - flat[:, 1].min()
    if yext > xext:  # rotate 90 deg about Z (swap X<->Y)
        R = R[:, :, [1, 0, 2]].copy()
        R[:, :, 0] *= -1
    # centre X,Y ; hull bottom (min Z) -> 0
...
# ---- pass 1: load all, shared world scale ----
data = {}
maxxy = 0.0; maxh = 0.0
for name in MODELS:
    ...
    print(f"{name:14s} len={flat[:,0].max()-flat[:,0].min():6.2f} height={flat[:,2].max():6.2f}")
L = max(2 * maxxy, maxh) * 1.06
...
# ---- shared union crop across all boats (nature render) ----
ux0 = uy0 = 10 ** 9; ux1 = uy1 = -1
for name in MODELS:
    bb = draw(name, hex_rgb(TRIBES["nature"])).getbbox()
    if bb:
...
for name in MODELS:
    tier = MODELS[name]
    out = os.path.join(ASSETS, tier); os.makedirs(out, exist_ok=True)
    for key, hx in TRIBES.items():
        im = draw(name, hex_rgb(hx)).crop(crop)
        im.save(os.path.join(out, f"{tier}_{key}.png"))
    print("saved", tier)
print("done")
