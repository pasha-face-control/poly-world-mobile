#!/usr/bin/env python3
"""Render the city-builder Material Factory + Trade Tower models to 2D isometric
sprites, matching the citadel/house pipeline (ortho, elev=30, azim=-45). These
are temporary pre-coloured models, so vertex colours are kept as-is. Output:
...
def draw(R, cols, hasc, L):
    ...
    for t, c, h in zip(R, cols, hasc):
        ...
        # Drop the flat ground base slab so only the structure sits on the grass.
        if ln > 0 and abs(n[2]) / ln > 0.90 and t[:, 2].max() < 0.10 * zmax:
            continue
...
for name in MODELS:
    ...
    flat = R.reshape(-1, 3)
    # Cube sized to the model so it fills the frame like the house sprite.
