...
## agent_communication:
##     -agent: "main"
##     -message: "ITERATION 22 — City Screen economy/limits update. (1) City Screen LEFT PILL now shows the city's star income +N/turn AND its population (testIDs city-income shows +cityStarIncome, city-population shows city.population). (2) A HOUSE road-connected to the citadel grants +2 population once (persistent, engine reconcileCityConnections) and +1★/turn while connected (engine cityStarIncome = production + connectedHouseCount; included in real turn income). (3) HOUSE cost changed to 5★ + 8 planks (data.ts). (4) CITADEL upgrades now also require city level: stage5 needs level4, stage10 needs level9, stage15 needs level14 (canUpgradeCitadel returns 'Requires city level N'); all star/goods costs unchanged. (5) Factory footprints are now per-tribe: Lesnoi Sawmill & Fishmen Glass 3x3; He-he Stone Quarry & Freemen Sand Quarry 4x4 (data.ts buildingSize; CityMap uses it for footprint/placement/hit-test; models auto-scale to footprint). (6) BUILDING LIMITS by citadel stage: stage1 = 1 house/1 factory/0 tower/0 park; stage5 = 4/2/1/2; stage10 = 8/2/2/4; stage15 = unlimited (canPlaceCityBuilding enforces, reason 'Building limit reached — upgrade citadel' or 'Upgrade citadel to unlock'). Engine node scripts/engine_test.js = 185/185 pass. 100% client-side, NO backend. FRONTEND-test the CITY SCREEN (390x844): New Game (menu-new-game) -> Single Player (mode-single) -> (Peaceful if shown) -> start-game -> skip tutorial -> tap capital (~195,420) -> Enter the City (enter-city). VERIFY: (A) left pill shows '+N /turn' AND a population count. (B) Open Buildings (city-buildings): at citadel stage 1, Trade Tower and Park should be locked/blocked (placing them shows an 'Upgrade citadel to unlock' toast); a Factory is placeable (FREE); a House now costs 5★ + 8 planks (likely unaffordable on a fresh capital — a placement attempt should toast 'Not enough planks/stars'). Placing a 2nd factory at stage 1 should be blocked ('Building limit reached'). (C) Open Citadel (city-citadel) -> the upgrade should show/deny with a 'Requires city level 4' style message since a fresh capital is below level 4. (D) Regression: place a Factory, then Edit->Move and Edit->Demolish still work; Exit + End Turn x2 no crash. Ignore benign RN Web shadow*/pointerEvents/resizeMode/tintColor warnings."
